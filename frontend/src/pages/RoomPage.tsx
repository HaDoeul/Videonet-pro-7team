...
// ⚠️ NativeWebRTCConnection 클래스에 replaceTrack 메서드가 있다고 가정합니다.
...
// RTCPeerConnection 타입은 브라우저 전역에 있으므로 import 불필요
...
export default function RoomPage() {
  ...
  // ✅ 2. Refs 추가 (Video Quality)
  const localFrameRef = useRef<ImageData | null>(null);
  // ✅ stats loop clearInterval Ref
  ...
  // ⭐️ [수정] 장치 선택을 고려하여 미디어 권한 요청 함수 업데이트
  ...
  // ⭐️ [추가] 로컬 스트림 업데이트 및 원격 참가자에게 트랙 교체 알림
  ...
  // ✅ 3. Q값을 WebRTC 인코더에 반영하는 함수 (오류 수정됨)
  const applyQToSenders = async (q: number) => {
    connectionsRef.current.forEach((conn) => {
      // ⭐️ 수정: conn.pc가 존재하지 않아 getSenders() 호출 시 오류 발생 방지
...
const createPeerConnection = async (userId: string, username: string, isInitiator: boolean, remoteIsHandRaised: boolean = false) => {
    ...
    // 2. 로컬 스트림을 연결하고 WebRTC 연결 준비
    await connection.connect(localStreamRef.current || undefined);
    connectionsRef.current.set(userId, connection);

    // ⭐️ 3. [핵심 수정] Initiator일 경우 Offer 생성 및 전송 ⭐️
    if (isInitiator) {
        try {
            // NativeWebRTCConnection 내부의 메서드를 호출하여 Offer 생성
            // (이 메서드는 pc.createOffer()와 pc.setLocalDescription(offer)를 포함해야 함)
            const offer = await connection.createOffer(); 
            
            // Socket.IO를 통해 상대방에게 Offer 전송
            socketRef.current?.emit("webrtc_offer", { to: userId, offer });
            console.log(`✉️ Offer 전송: ${socketIdRef.current} -> ${userId}`);
            
            // Offer를 보냈으니 통계 수집 및 품질 설정 시작
            applyQToSenders(Q);
...
const handleWebRTCOffer = async (from: string, offer: RTCSessionDescriptionInit) => {
    ...
    if (!connection) {
      ...
      // connect는 peerConnection을 초기화하고 로컬 스트림을 트랙으로 추가합니다.
      await connection.connect(localStreamRef.current || undefined);
      
      // ⭐️ 핵심 수정 1: 새로 생성된 연결 객체를 맵에 저장합니다.
      connectionsRef.current.set(from, connection); 
    }
    
    // 2. RTCPeerConnection 객체 가져오기 (Getter 사용)
    // connection 객체의 peerConnection getter를 사용하여 내부 인스턴스에 접근합니다.
    ...
    // 중복 Offer 방지: signalingState가 "stable"이 아닌 경우는 무시합니다.
    // 이는 Answer를 처리해야 할 Offer 수신자 측 로직이므로, "stable" 상태에서 Offer를 받는 것이 정상입니다.
...
const handleWebRTCAnswer = async (from, answer) => {
  ...
  // ✅ 6. SSIM 계산 함수 추가 (simple grayscale approx)
  ...
  // ... (toggleScreenShare, restoreOriginalVideo, leaveRoom, cleanup, sendMessage, getGridClass, getVideoTileClass - 기존 로직 유지)
